...
- **Env var naming**: every dridock setting is `DRIDOCK_X` and **only** `DRIDOCK_X`. The `CLAUDEBOX_*` (2.x) and `CLAUDE_*` (upstream 1.x) tiers, the `env-rename.map`, and the container aliaser were all removed in **5.0.0** — do not re-add a fallback chain. Genuine upstream Claude Code variables (`CLAUDE_CODE_OAUTH_TOKEN`, `CLAUDE_CONFIG_DIR`, `ANTHROPIC_API_KEY`) were never part of that rename and are read by name as normal. When adding a setting: document it in `docs/environment-variables.md`, and if it's a common user-facing opt-in add it to the `--help` **USEFUL ENV** list too (the full doc is exhaustive; `--help` is the curated subset — keep both in sync).
- **Planned removals live in [docs/roadmap.md](docs/roadmap.md), and each one is enforced by a test.** Before promising that something "goes away in X.0", add the row AND a check in `tests/test_deprecation_deadlines.sh`. This exists because `entrypoint.sh` told every claudebot that legacy env names would go away in 4.0, we shipped through 4.4.0 with the shim still live, and nothing noticed until a namespace sweep (#82). A prose deadline does not hold; a test that fails the moment the release arrives does. **6.0.0 removes the `.claudebox/` → `.dridock/` migration path** (the `migrate` verb and the migrators).
...
- **Versioning is a host↔image contract, not decoration.** The fork's semver lives in the `VERSION` file, is embedded as `DRIDOCK_TS_VERSION` in `dridock-ts/src/domain/dridockVersion.ts`, and is stamped into the image at build time (`Dockerfile` `ARG`/`ENV`/`LABEL org.dridock.version`, passed by `make`/`install.sh`). **Bump it whenever you change the host↔container IPC contract** — sidecar filenames/formats, forwarded env, `/out`, secrets injection — so `dridock checkversion` (host wrapper vs the project's image label) can warn on drift. Bump `VERSION` and `dridockVersion.ts` together (both need to match at build time). On every bump: add a `CHANGELOG.md` entry and tag the commit `git tag -a vX.Y.Z`. Run **`make hooks`** once per clone: it installs a pre-commit hook that refuses direct commits on `master` (merges excepted). This is enforced rather than trusted because it failed in practice — a small fix went straight to master mid-session on 2026-09-01 out of momentum, not intent. Work branches are named **`<issue#>-<slug>`** after the GitHub issue they close (`62-mcp-pin-api-mode`), one issue per branch, with the issue referenced in every commit (`fix(#62): …`). **Whoever owns the branch merges it** and says so on the issue; the reviewer signs off but doesn't merge. **When claiming something is verified, name the environment** (macOS+colima vs Linux+docker backend) and say what you didn't cover — this fork ships into both and #63/#66 were each invisible from one side. Full release/SDLC process — semver rules, release steps, branch naming, changelog policy — lives in **[docs/versioning.md](docs/versioning.md)**.
...
- **A claim must carry its evidence — and never end a turn stating an intention.** A turn ends when you stop making tool calls, so "running the suite now" as a closing line is a claim about a future that turn cannot reach: the work never starts, and the next message reports a result that doesn't exist. Do it FIRST, then describe it. Quote what proves it — a task id, a URL, a commit sha, the actual command output. `"suite running (task b38tx12i9)"` is checkable; `"running the suite now"` isn't, and you cannot cite a task id for something you never launched. Past tense for done, explicit conditional for pending (`"next is X — say go"`), never present-progressive for something unstarted. This is the root of a failure class this project keeps hitting from both agents: `team post` that composed but never sent, a completion report naming a branch that wasn't pushed, "verified" that never named the runtime, a `git push` reporting *Everything up-to-date* from the wrong branch. In every case the **report** and the **thing reported** came from separate steps with nothing forcing them to agree. Before asserting anything about state, ask what output would prove it and quote that; if nothing can be quoted, the claim isn't ready to make.
- **Asking Alan to act on your behalf: WHERE/TYPE blocks, never prose.** Agents can't reach each other's sessions, accounts, or containers (Arfy runs as `claude-arfy`, Bear under `aberezin`, each with its own colima profile — so a `make build` in one account is invisible to the other; see the housekeeping note on #58). When you need Alan to run something for you, give **only** the target and the literal string to type, one block per step, in order — no prose describing the step. Named targets: **Bear's Claude prompt**, **Alan's bash (aberezin)**, **Bear's container bash**. Anything you can run yourself, run — don't ask. Verify the step is actually needed before asking: a prose description makes Alan reconstruct the command, and an unnecessary step wastes a round trip (a `dridock start` was requested on #58 for a container that was already up). Format:

  > **WHERE:** Bear's Claude prompt
  > **TYPE:** `/exit`
...
- **Every doc must be listed in `docs/README.md`** — enforced by `tests/test_docs_index.sh`, which also fails on a dead link in that index. Adding a doc and indexing it are separate steps and nothing forced the second: an audit on 2026-09-02 found six missing, two written that same day by the person auditing. An unindexed doc is findable only by someone who already knows it exists.
...
- **Diagnostics must be derived from the CURRENT run's state, not from accumulated state that merely contains it.** Sister-rule to the never-silently-discard rule above. A diagnostic that reads from an append-only or long-lived source (a fetcher log, a shared cursors file, a state directory that outlives a specific run) will find stale evidence from previous runs and report it as if it were current — asserting the OPPOSITE of the truth on every run after the first. This is the class Arfy caught three times in the #56/#58 review chain: (1) the drain-note going to stderr (correct signal, wrong channel), (2) `rm cursors + re-arm` advice landing in the EOF fallback (correct instruction, wrong actionable result), (3) surfacing FRESH START by greping the whole fetcher log (correct pattern, wrong provenance — a stale FRESH START from a dead lifetime got re-announced on every genuine-persistent spawn). The pattern to reach for: capture a "before" watermark (byte offset, timestamp, cursor position) and slice only the current run's window (`tail -c +$((_before + 1))`, `--since=$_before_iso`, etc.). Same shape as the drain + Monitor-arm idioms already used elsewhere. Reproduced INSIDE the fix for #58 once (0b93ab5 → 27a4a01 → had to add byte-slicing in the follow-up), so this rule is fresh evidence, not archaeology.
