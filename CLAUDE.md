...
- **Env var naming**: always read as `${CLAUDEBOX_X:-${CLAUDE_X:-default}}`. When adding a new setting, follow this dual-prefix pattern and document it in `docs/environment-variables.md` — and, if it's a common user-facing opt-in, also add it to the `--help` **USEFUL ENV** list in `wrapper.sh` (the full doc is exhaustive; `--help` is the curated subset — keep both in sync).
...
- [docs/versioning.md](docs/versioning.md) — semver, releases, and `claudebox checkversion` (host↔image drift).
