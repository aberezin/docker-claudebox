...
- **Container-role naming is load-bearing.** The `_prog` / `_cron` suffixes and the `.<container>-auth` / `-args` / `-update` / `-no-continue` sidecar files in `~/.claude/` are the IPC mechanism between the host wrapper and restarted containers. Keep the naming in `wrapper.sh` and `entrypoint.sh` in sync.
