...
describe("scanOrphans — the core cwd→slug→foreign-id walk", () => {
  ...
  test("returns empty when no sibling projects match this cwd slug", async () => {
    ...
    fs.seed("/home/alan/.config/dridock/projects/aabbccdd/dot/.claude/projects/-other/x.jsonl", "{}");
    const orphans = await scanOrphans(
      { fs, env: process.env, home: "/home/alan" },
      "/repo",
      undefined,
  ...
  test("lists foreign ids whose session dir matches this cwd, sorted-stable by scanner order", async () => {
    setXdg("/home/alan/.config");
    const fs = new InMemoryFileSystem();
    fs.seed("/home/alan/.config/dridock/projects/69adc719/dot/.claude/projects/-repo/session-1.jsonl", "{}");
    fs.seed("/home/alan/.config/dridock/projects/69adc719/dot/.claude/projects/-repo/session-2.jsonl", "{}");
    fs.seed("/home/alan/.config/dridock/projects/aabbccdd/dot/.claude/projects/-repo/session-x.jsonl", "{}");
    const orphans = await scanOrphans(
      { fs, env: process.env, home: "/home/alan" },
      "/repo",
      undefined,
    ...
    expect(first.path).toBe("/home/alan/.config/dridock/projects/69adc719/dot/.claude/projects/-repo");
  ...
  test("EXCLUDES ownId — the current project's own dir is not an orphan of itself", async () => {
    setXdg("/home/alan/.config");
    const fs = new InMemoryFileSystem();
    fs.seed("/home/alan/.config/dridock/projects/mine1234/dot/.claude/projects/-repo/session.jsonl", "{}");
    fs.seed("/home/alan/.config/dridock/projects/deadbeef/dot/.claude/projects/-repo/session.jsonl", "{}");
    const orphans = await scanOrphans(
      { fs, env: process.env, home: "/home/alan" },
      "/repo",
      "mine1234",
  ...
  test("SKIPS empty dirs — a clobbered-but-never-used sibling produces no signal", async () => {
    ...
    fs.seed("/home/alan/.config/dridock/projects/real0001/dot/.claude/projects/-repo/session.jsonl", "{}");
    // Empty dir — clobber remnant — must NOT surface.
    // (InMemoryFileSystem: seed a marker then delete/clear it — or mkdir directly.)
    await fs.mkdirRecursive("/home/alan/.config/dridock/projects/empty002/dot/.claude/projects/-repo");
    const orphans = await scanOrphans(
      { fs, env: process.env, home: "/home/alan" },
      "/repo",
      undefined,
  ...
  test("respects XDG_CONFIG_HOME override", async () => {
    setXdg("/custom/xdg");
    const fs = new InMemoryFileSystem();
    fs.seed("/custom/xdg/dridock/projects/aa/dot/.claude/projects/-repo/s.jsonl", "{}");
    const orphans = await scanOrphans(
    ...
    expect(orphans[0]!.path).toBe("/custom/xdg/dridock/projects/aa/dot/.claude/projects/-repo");
  });
});
...
describe("formatMintWarning — the bootstrap-mint text", () => {
  test("lists each orphan with entry count, plus the adopt recovery hint", () => {
    const lines = formatMintWarning([
      { id: "aa", path: "/xdg/dridock/projects/aa/dot/.claude/projects/-repo", entryCount: 5 },
      { id: "bb", path: "/xdg/dridock/projects/bb/dot/.claude/projects/-repo", entryCount: 1 },
    ]);
    const text = lines.join("");
    expect(text).toContain(`⚠️  bootstrap: minting a NEW project id will silently orphan`);
    expect(text).toContain(`/xdg/dridock/projects/aa/dot/.claude/projects/-repo   (5 entries)`);
    expect(text).toContain(`/xdg/dridock/projects/bb/dot/.claude/projects/-repo   (1 entries)`);
...
describe("formatLaunchWarning — the start/cron text", () => {
  test("shows the launching id vs the orphaned ids for immediate context", () => {
    const lines = formatLaunchWarning("current1", [
      { id: "orphaned1", path: "/xdg/dridock/projects/orphaned1/dot/.claude/projects/-repo", entryCount: 12 },
    ]);
    const text = lines.join("");
    ...
    expect(text).toContain(`/xdg/dridock/projects/orphaned1/dot/.claude/projects/-repo   (12 entries)`);
