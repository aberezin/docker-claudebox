...
| `CLAUDEBOX_FRAMEWORK_DEV`  | Set to `1` to force the entrypoint's framework-dev startup surfacing (cross-project consults awaiting a framework draft + unreviewed framework-bug reports, listed via `cb-consult list --all` / `cb-report-bug list`). Auto-detected when the workspace looks like a claudebox harness fork (a `wrapper.sh` at its root containing `CLAUDEBOX_VERSION=`); this env is the explicit opt-in for a renamed/relocated fork. | _(auto)_ |
