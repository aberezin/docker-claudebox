...
| `DRIDOCK_NO_OAUTH_TOKEN`   | Set to `1` to **never** send `CLAUDE_CODE_OAUTH_TOKEN` (setup-token style) into the container. Needed when you've done full-scope `claude auth login` inside the container and want the stored login (which has RC/mobile-push scope) to take effect — otherwise the model-scope env var overrides it and Remote Control refuses to activate. See [design/git-and-api-auth.md § Claude Code auth](design/git-and-api-auth.md#claude-code-auth-distinct-from-gitapi-auth-above) and [Issue #16](https://github.com/aberezin/docker-claudebox/issues/16). | _(off)_ |
