...
| `DRIDOCK_HOST_AGENT_PORT` / `DRIDOCK_HOST_AGENT_BIND` | Tune the opt-in host agent (`dridock host-agent up`, Approach 2 / #15) — the port and bind address it listens on. **BIND stays the Colima gateway; never set it to `0.0.0.0`/a LAN address.** See [design/backends.md](design/backends.md). | `9280` / `192.168.64.1` |
