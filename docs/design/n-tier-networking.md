# N-tier networking under claudebox — addressing, binding, and CORS

**Status:** Standard / accepted — produced by the first [framework consult](framework-consult.md).
**Applies to:** any multi-tier app built inside a claudebox project (per-project Colima VM). Builds on [per-project-vm.md](per-project-vm.md) and [browser-testing.md](browser-testing.md).

## Summary

An N-tier app under claudebox has **two kinds of traffic that use two different address spaces**, and the browser-facing one **rotates**. Almost every "worked yesterday, 403/blocked/refused today" bug is one plane's address leaking into the other, or the rotating address hardcoded into source.
...
- **Browser → service** (the human's Chrome; the Approach-B `cb-browser cdp` browser) runs *on the Mac* → it reaches a workload only at the **VM's reachable IP**, `http://$CLAUDEBOX_VM_IP:<port>`. **Rotates across VM restarts** (a real case: `.13` → `.16`).

Keep the planes straight, never hardcode the rotating one, bind `0.0.0.0`, and drive CORS/allowed-origins from `$CLAUDEBOX_VM_IP` (or the stable `network.hostname`). That's the whole standard.
...
| Address to use | **container name** — `http://api:8080`, `postgres:5432` | **`http://$CLAUDEBOX_VM_IP:<port>`** (published port) |
...
The claudebot container sits on the VM's docker **bridge** (`172.x`) and **cannot self-discover** the VM's reachable `192.168.64.x` (`col0`) address. The harness injects it as **`$CLAUDEBOX_VM_IP`**, refreshed on **every** launch (via a durable `-vmip` sidecar, the same self-healing pattern as the auth/secrets/`-cdp` sidecars), so it always tracks rotation.

- In-container: `echo "$CLAUDEBOX_VM_IP"` or `cb-browser ip`.
- On the Mac: `claudebox ip` (`claudebox net` prints the full dashboard).
...
**Rotation-proof escape hatch:** set a stable name once with `claudebox net <name>` — it writes `network.hostname` into `.claudebox/config.yml` and prints an `/etc/hosts` line for the human (`<vm-ip>  <name>`) to paste. The harness then also exposes it in the container as **`$CLAUDEBOX_HOSTNAME`**. The browser uses `http://<name>:<port>`, which survives rotation (the human re-points the hosts entry when the wrapper flags it stale; `claudebox net` re-prints it). claudebox never edits `/etc/hosts` itself.
...
**(a) Drive the allowlist from `$CLAUDEBOX_VM_IP` at server start.** The env var is present in the claudebot and inherited by workloads you launch with `docker run -e CLAUDEBOX_VM_IP` (or an env-file). Build the origin list at boot:
...
vm_ip = os.environ.get("CLAUDEBOX_VM_IP", "")
origins = [f"http://{vm_ip}:3000"] if vm_ip else []
origins += ["http://localhost:3000"]           # colima-forwarded fallback
# optional stable name (claudebox net <name>):
if host := os.environ.get("CLAUDEBOX_HOSTNAME"):
...
const vmIp = process.env.CLAUDEBOX_VM_IP;
const origins = [
  vmIp && `http://${vmIp}:3000`,
  "http://localhost:3000",
  process.env.CLAUDEBOX_HOSTNAME && `http://${process.env.CLAUDEBOX_HOSTNAME}:3000`,
].filter(Boolean);
...
**(c) Stable hostname.** With `claudebox net <name>` set, pin `http://<name>:3000` in the allowlist and be done — no env, no rotation.
...
const nextConfig = {
  allowedDevOrigins: [
    ...
    "localhost",
    ...(process.env.CLAUDEBOX_HOSTNAME ? [process.env.CLAUDEBOX_HOSTNAME] : []),
  ],
};
...
//   NEXT_PUBLIC_API_BASE="http://$CLAUDEBOX_VM_IP:8080"
...
docker run -d --name web --network cb-net -p 3000:3000 \
  -e CLAUDEBOX_VM_IP \
  -e NEXT_PUBLIC_API_BASE="http://$CLAUDEBOX_VM_IP:8080" \
...
#   → browser→API:   http://$CLAUDEBOX_VM_IP:8080   (browser plane, published)
...
#   → browser→web:   http://$CLAUDEBOX_VM_IP:3000    (browser plane)
#   → browser→API:   http://$CLAUDEBOX_VM_IP:8080    (browser plane)
```
...
| API | `0.0.0.0:8080` | `postgres:5432` | `http://$CLAUDEBOX_VM_IP:8080` | **yes** `-p 8080:8080` |
| Next (server) | `0.0.0.0:3000` | `http://api:8080` | — | — |
| Next (browser) | — | — | web `http://$CLAUDEBOX_VM_IP:3000`, API via `NEXT_PUBLIC_API_BASE` | **yes** `-p 3000:3000` |

Where the harness plugs in: `$CLAUDEBOX_VM_IP` (fresh every run) feeds the API CORS list, Next `allowedDevOrigins`, and `NEXT_PUBLIC_API_BASE`. For a rotation-proof human URL, `claudebox net <name>` adds a stable hostname (also `$CLAUDEBOX_HOSTNAME` in-container).
...
- **Human's real Chrome (opt-in, Approach B):** `claudebox browser-bridge up` on the Mac, then `cb-browser cdp http://$CLAUDEBOX_VM_IP:3000`. The browser is on the Mac, so the URL **must** be the VM IP — `cb-browser cdp` auto-rewrites a `localhost` URL to `$CLAUDEBOX_VM_IP`, but pass it directly. See [browser-testing.md](browser-testing.md).

## Troubleshooting — symptom → cause → fix

| Symptom | Cause | Fix |
|---|---|---|
| API returns **403 / "origin not allowed"** on the browser's request | CORS allowlist doesn't contain the browser's (rotating VM-IP) Origin | drive the allowlist from `$CLAUDEBOX_VM_IP` (Rule 3a), reflect the `192.168.64.0/24` subnet (3b), or use a stable hostname (3c) |
| Next dev prints **"Blocked cross-origin request"** | VM IP not in `allowedDevOrigins` (or a stale one pinned) | compute `allowedDevOrigins` from `$CLAUDEBOX_VM_IP` at config load (Rule 1) |
| **`ERR_CONNECTION_REFUSED` on `localhost:<port>`** in the human's browser | addressed the Mac loopback, not the VM; or colima isn't forwarding that port | use `http://$CLAUDEBOX_VM_IP:<port>` (`claudebox ip` / `cb-browser ip`) |
...
| **Worked, then broke after a VM restart** | a literal `192.168.64.x` was hardcoded and the IP rotated | purge hardcoded IPs; read `$CLAUDEBOX_VM_IP` fresh, or pin a `claudebox net <name>` hostname (Rule 1) |
...
| **Websocket blocked / fails to connect** from the browser | `ws://` pointed at `localhost` or a `cb-net` name, or Origin rejected | point the socket at `ws` on `$CLAUDEBOX_VM_IP:<port>`; include that Origin in the server's allowlist (Rules 3–4) |
...
- **claudebox (this project):** injects `$CLAUDEBOX_VM_IP` (and `$CLAUDEBOX_HOSTNAME` when set) fresh every run, provides `cb-net`, `cb-browser`, `claudebox ip`/`net`, and the stable-hostname mechanism; bakes this standard into the container `CLAUDE.md` so every claudebot enforces it identically.
- **A project's Claude:** keep the two planes straight, bind `0.0.0.0`, read `$CLAUDEBOX_VM_IP` fresh (never hardcode), and configure CORS/allowed-origins from it or the hostname. Don't invent ad-hoc port maps or paste a past IP.
